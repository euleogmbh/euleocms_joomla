 ...
 *
*/
include_once(JOOMFISH_ADMINPATH . DS .'models' . DS . 'JFContent.php');
...
class ContentObject {
	...
	function bind( $formArray, $prefix="", $suffix="", $tryBind=true, $storeOriginalText=false ) {
		...
		for( $i=0; $i<count($elementTable->Fields); $i++ ) {
			...
			if( isset($formArray[$prefix ."refField_". $fieldName .$suffix]) ) {
				// we dont need to check magic quotes because we got the data via soap
				...
				$fieldContent->published = $formArray[$prefix . 'published_' . $fieldName . $suffix];
	...
	function readFromRow( $row ) {
		...
		if (isset($row->attribs)) $this->attribs = new JParameter($row->attribs);
